    ...
    <div className="task-manager p-6 bg-gray-800 h-screen min-w-min">
      ...
      <table className="border-solid w-full">
        ...
        {filteredTasks
            ...
            .map((task) => (
            <tr key={task.id} className={changedRows.has(task.id) ? 'bg-green-900' : ''}>
              ...
              <td className="border border-gray-400 py-2 px-1.5 text-center text-white">
                {editableTaskId === task.id ? (
                  <input
                    ...
                    className={changedRows.has(task.id) ? 'bg-green-900 h-full w-full text-center outline-none' : 'h-full w-full text-center outline-none bg-gray-800'}
                    onChange={(e) => {
                ...
                {editableTaskId === task.id ? (
                    <div className="relative">
                      <input
                        type="date"
                        value={task.deadline}
                        className={changedRows.has(task.id) ? 'bg-green-900 text-white outline-none' : 'bg-gray-800 text-white outline-none'}
                        onChange={(e) => {
                          const updatedTasks = tasks.map((t) =>
              ...
              <td className="border border-gray-400 py-2 px-1.5">
                {editableTaskId === task.id ? (
                  <div className="flex items-center">
                    ...
                    <div className="w-3.5">
                      <select
                        className={changedRows.has(task.id) ? 'bg-green-900 border-0 outline-none justify-start py-1 w-3.5 appearance-none text-white' : 'border-0 outline-none justify-start py-1 w-3.5 appearance-none bg-gray-800 text-white'}
                        onChange={(e) => {
                          const selectedId = parseInt(e.target.value);
              ...
              <td className="border border-gray-400 py-2 px-1.5 text-center">
                {editableTaskId === task.id ? (
                  <select
                    value={task.status}
                    className={changedRows.has(task.id) ? 'bg-green-900 border-0 outline-none w-full py-1 rounded text-center text-white' : 'border-0 outline-none w-full py-1 rounded text-center bg-gray-800 text-white'}
                    onChange={(e) => {
          ...
          <tr>
            ...
            <td className="border border-gray-400 py-2 px-1.5 text-center">
              <input
                ...
                className="h-full w-full text-center outline-none bg-gray-800 text-white"
                onChange={(e) => setNewTask({ ...newTask, description: e.target.value })}
              />
            </td>

            <td className="border border-gray-400 py-2 px-1.5 text-center relative">
                <input
                    type="date"
                    value={newTask.deadline}
                    className={`h-full w-full text-center outline-none pl-5 bg-gray-800 ${
                    newTask.deadline ? 'text-white' : 'text-gray-600'
                    }`}
            ...
            <td className="border border-gray-400 py-2 px-1.5 relative">
              <div className="flex items-center">
                <div className="flex-1 px-0">
                ...
                <div className="w-3.5">
                  <select
                    className="border-0 outline-none justify-start py-1 w-3.5 appearance-none bg-gray-800 text-white"
                    onChange={(e) => {
                      const selectedId = parseInt(e.target.value);
                      const foundEmployee = employees.find(
            ...
            <td className="border border-gray-400 py-2 px-1.5 text-center">
              <input
                ...
                className="h-full w-full text-center outline-none pl-4 text-white bg-gray-800"
                min="1"
                max="3"
                onChange={(e) => {
                  const priority = parseInt(e.target.value);
